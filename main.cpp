...
//     static AudioBuffer buffer(1000);  // Store 1000 samples
//     static float t = 0;
//     static float history = 5.0f;  // Show 5 seconds of history

//     // Generate and push new audio sample
//     float new_sample = generateAudioSample(t);
//     buffer.push(new_sample);
//     t += 1.0f / 44100;  // Assume 44.1kHz sample rate
...
//         std::vector<float> x(buffer.max_size), y(buffer.max_size);
//         for (int i = 0; i < buffer.max_size; i++) {
//             int idx = (buffer.offset - buffer.max_size + i + buffer.max_size) % buffer.max_size;
//             x[i] = t - (buffer.max_size - i) * (1.0f / 44100);
//             y[i] = buffer.data[idx];
//         }

//         ImPlot::PlotLine("Waveform", x.data(), y.data(), buffer.max_size);
//         ImPlot::EndPlot();
//     }
// }
...
void ShowAudioWaveform() {
    ...
    if (ImPlot::BeginPlot("Audio Waveform")) {
        ...
        std::vector<float> x, y;
        {
            ...
            size_t num_samples = std::min(audioData.buffer.size(), size_t(history * 44100));
